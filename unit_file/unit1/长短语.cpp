...
int error_sum = 0;
void print_all(std::vector<xzs> &xz) {
    for (const auto &tmp : xz) {
        LOG(tmp.num, "    ", tmp.daan, "\n");
    }
}
int main() {
    LOG("mat might");
    ...
    */
    ...
    xz.push_back({"感染疾病", "to contract a sickness/an illness"});
    xz.push_back({"关心的神色", "a look of concern"});
    xz.push_back({"错过表演", "to miss the performance"});
